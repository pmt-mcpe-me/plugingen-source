 ...
 *
 * @author PocketMine Team (modified by PEMapModder)
...
class PluginDescription{
	...
	public $website = null;
	public $prefix = null;
	public $order = PluginLoadOrder::POSTWORLD;

	public $permissions = [];

	public function __construct($data, $yaml = true){
		$this->loadMap($yaml ? \yaml_parse($data) : $data);
	}
	...
	private function loadMap(array $plugin){
		...
		if(stripos($this->main, "pocketmine\\") === 0){
			throw new \Exception("Invalid PluginDescription main, cannot start within the PocketMine namespace");
		}
		...
		if(isset($plugin["load"])){
			...
			if(!defined(PluginLoadOrder::class . "::" . $order)){
				throw new \Exception("Invalid PluginDescription load");
			}else{
